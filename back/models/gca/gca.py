...
class GCA(object):
    ...
    def __init__(self):
        with open(os.path.join(os.path.dirname(__file__), 'config/gca-dist-all-data.toml')) as f:
        ...
        # build model
        self.model = networks.get_generator(encoder=CONFIG.model.arch.encoder, decoder=CONFIG.model.arch.decoder)
        # self.model.cuda()

        # load checkpoint
        checkpoint = torch.load(os.path.join(os.path.dirname(__file__), 'gca-dist-all-data.pth'))
        self.model.load_state_dict(utils.remove_prefix_state_dict(checkpoint['state_dict']), strict=True)

        # inference
        self.model.eval()

    def inference(self, image, trimap):
        image_dict = generator_tensor_dict(image, trimap)
        pred, offset = single_inference(self.model, image_dict)
        return pred
